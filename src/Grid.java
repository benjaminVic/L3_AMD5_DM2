	...
	public Grid(int n, int m) {
		...
		for (int i = 0; i < n; i++)
			for (int j = 0; j < m; j++) {
				...
				/* Set EAST */
				this.cell[i][j].setNeighbor(Cellule.EAST,
						(j == this.columns - 1) ? null : this.cell[i][j + 1]);

				/* Set WEST */
				this.cell[i][j].setNeighbor(Cellule.WEST, (j == 0) ? null
						: this.cell[i][j - 1]);
				...
				if (i == this.rows - 1)
					...
					this.cell[i][j].setNeighbor(Cellule.SOUTHEAST,
							this.cell[i + 1][j]);
				...
				if (i == this.rows - 1)
					...
					this.cell[i][j].setNeighbor(Cellule.SOUTHWEST,
							this.cell[i + 1][j - 1]);
				else if (i % 2 == 1)
					this.cell[i][j].setNeighbor(Cellule.SOUTHWEST,
							this.cell[i + 1][j]);
				...
				if (i <= 0)
					...
					this.cell[i][j].setNeighbor(Cellule.NORTHEAST,
							this.cell[i - 1][j]);
				else if (i % 2 == 1 && j < this.columns - 1)
					this.cell[i][j].setNeighbor(Cellule.NORTHEAST,
							this.cell[i - 1][j + 1]);
				...
				if (i <= 0)
					...
					this.cell[i][j].setNeighbor(Cellule.NORTHWEST,
							this.cell[i - 1][j - 1]);
				else if (i % 2 == 1)
					this.cell[i][j].setNeighbor(Cellule.NORTHWEST,
							this.cell[i - 1][j]);
	...
	public void showGrid(boolean show) {
		...
		for (int i = 0; i < this.rows; i++) {
			if (i % 2 == 1)
				System.out.print("  ");
			for (int j = 0; j < this.columns; j++)
				System.out.print(((this.cell[i][j].getNeighbor(Cellule.WEST)
						.isWall() && (i != 0 || j != 0)) ? "|" : " ")
